...
T_DISK=$(df -m --output=size --total | tail -1 |cut -c 7-)
T_DISK=$((T_DISK / 1000))
P_DISK=$((100 * U_DISK / (T_DISK * 1000)))

L_BOOT=$(who -b | awk '{$1=$2="";print $0}' | cut -d " " -f3-)

LVM_S=$(lsblk | grep "lvm" | wc -l)
LVM_A=$(if [ $LVM_S -eq 0 ]; then echo no; else echo yes; fi)
...
echo    "#Disk Usage : ${U_DISK}MB/${T_DISK}G ($P_DISK%)"
echo    "#CPU load : $CPU_L%"
